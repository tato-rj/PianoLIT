...
### 2026-09-24 — Fit printed score pages to the paper (implemented locally)

- Priority: P2 print usability. A Chrome print preview of the three-page Adagio Patetico PDF showed the score much smaller than its source page. The print renderer had capped each page at 190 mm wide and 245 mm tall, leaving unnecessary whitespace. Each generated print sheet now uses 98% of the available paper width and keeps the PDF page's aspect ratio; the slight inset avoids an extra sheet from print-pagination rounding. The score canvas and annotation SVG both fill that same sheet, so markings stay aligned. Print margins are removed for the score layout.
- Affected files: score-editor JavaScript/SCSS, tracked editor JavaScript/app CSS/manifest, and JavaScript regression tests. Original PDF downloads and annotation storage are unchanged.
- Verification: the three-page browser fixture produced print sheets with the source PDF's 595:842 aspect ratio. Chrome's native print preview showed three pages with the notation spanning roughly 90% of the paper width and no extra sheets. Focused JavaScript regressions and an isolated production asset build passed. Physical printer output and other browser print dialogs still need a smoke check.

### 2026-09-24 — Match score print paper to the PDF (implemented locally)

- Priority: P2 print fidelity. The downloaded score's native PDF print preview uses the source PDF's page shape, while the web page Print button used the browser's default paper shape. On a wider score PDF such as Pierrot and Pierrette, this left a large empty area below the music. Print now adds a temporary `@page size` matching the PDF's first page and removes that rule after printing. The 98% inset still prevents pagination rounding from creating an extra sheet; annotation layers retain their alignment.
- Affected files: score-editor JavaScript/SCSS, tracked editor JavaScript/app CSS/manifest, and JavaScript regression tests. Downloads, stored annotations, and mobile APIs are unchanged. The print-specific paper rule no longer applies to unrelated web pages.
- Verification: a focused regression checks a 612 × 792 pt PDF page requests that exact paper size and cleans up the temporary rule; the JavaScript suite and isolated production asset build pass. Browser print preview for the new paper-size rule remains to be checked on the actual piece, and physical printer output remains untested.
