<div data-name="{{$names[0]}}" data-octave="{{$octave}}" data-names="{{json_encode(enharmonic()->find($names[0]))}}" class="keyboard-white-key keyboard-key h-100 border rounded-bottom position-relative d-flex align-items-end justify-content-center pb-2 shadow-sm" style="width: 40px; cursor: pointer;">
	@if($names[1])
