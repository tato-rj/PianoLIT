  ...
  <tbody>
  	...
  	@foreach($history->receipt as $field => $value)
	  	...
	  	@include('admin.pages.users.show.list-item', [
	  		...
	  		'value' => (strpos($value, 'GMT') !== false) ? carbon(stripcslashes($value))->timezone(config('app.timezone'))->format('M dS, Y \a\t h:i A') : $value])
	  	@endif
  	@endforeach
  </tbody>
</table>

<div class="accordion mb-4" id="membership-history-receipts">
	...
	<div class="card">
		...
		<div id="receipt-history-{{$loop->iteration}}" class="collapse" aria-labelledby="receipt-{{$loop->iteration}}" data-parent="#membership-history-receipts">
				...
				<table class="table table-hover table-sm table-borderless m-0">
					<tbody>
						@foreach($receipt as $field => $value)
						  	@include('admin.pages.users.show.list-item', [
						  		'title' => ucfirst(str_replace('_', ' ', $field)), 
						  		'value' => (strpos($value, 'GMT') !== false) ? carbon(stripcslashes($value))->timezone(config('app.timezone'))->format('M dS, Y \a\t h:i A') : ucfirst($value)])
						@endforeach
					</tbody>
				</table>
			</div>
