@component('components.overlays.modal', ['title' => 'Neighbor Keys', 'size' => 'lg'])
	...
	<p>The <i>neighbor keys</i> are those keys that have only <strong>one note different from the main key</strong>, therefore are very similar and considered to be close. The more steps you move in either direction on the circle of fifths, the more distant the keys get from the main key (the more different they will sound).</p>
	...
	<p>If a piece modulates (changes keys), chances are it will move to one of its neighbors. This is because you just need to <u>change one note to move into the new key</u>, making it easier and seamless. It is not at all uncommon to modulate to more distant keys, of course, but it is more difficult to do that. Composers will often move gradually, going from one neighbor key to the next, until they reach the distant new key.</p>
