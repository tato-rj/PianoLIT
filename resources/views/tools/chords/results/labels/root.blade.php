...
<div class="p-2 text-muted">
	...
	<p>Here, the <strong>root {{chordToHumans($inversion['chord'][0])}}</strong> forms 
		...
		@if(! $loop->last)
		a <strong>{{$inversion['intervals'][$index]['name']}}</strong> with the {{iterationToHumans($loop->iteration)}} note {{chordToHumans($inversion['chord'][$loop->iteration])}}{{$loop->iteration < count($inversion['chord']) - 1 ? ',' : '.'}}
		@endif
		@endforeach
	</p>
</div>