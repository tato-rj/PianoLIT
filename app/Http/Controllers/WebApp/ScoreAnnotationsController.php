...
class ScoreAnnotationsController extends Controller
{
    ...
    public function update(Request $request, Piece $piece)
    {
        ...
        $data = $request->validate([
            ...
            'marks.*.type' => ['required', Rule::in(['stroke', 'text'])],
            'marks.*.page' => 'required|integer|min:1|max:2000',
            'marks.*.color' => ['required', 'string', 'regex:/\A#[a-fA-F0-9]{6}\z/'],
            'marks.*.width' => 'required_if:marks.*.type,stroke|numeric|between:0.001,0.02',
            'marks.*.points' => 'required_if:marks.*.type,stroke|array|min:1|max:1500',
